...
const fetchEvents =  ({addEvents, eventsFailed, eventType}) => {
  ...
  console.log(status)
  const url = eventType===3 ? "http://localhost:3001/events" : `http://localhost:3001/events?status=${status}`
  return fetch(url)
  .then(response => {
          if (response.ok) {
              return response;
...
class Home extends React.Component {
  ...
  toggleEventData = () => {
    ...
    fetchEvents({addEvents: this.addEvents, eventsFailed:this.eventsFailed , eventType:this.state.eventType})
  }
