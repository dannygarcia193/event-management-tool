...
const Pagination = ({props}) => {
    const {controlledPageCount,cardData, fetchData, pageIndex, 
        pageSize,canPreviousPage, canNextPage, previousPage, nextPage,
         gotoPage,pageCount} = props
    ...
    return(
        <div className="pagination">
            ...
            <button onClick={() => previousPage(pageIndex)} disabled={!canPreviousPage}>
            {'<'}
            </button>{' '}
...
const CardContainer = ({cardData})  => {
    ...
    const resetAndSetPageSize = pageSize => {setPageSize(pageSize);setPageIndex(0)}
    const controlledPageCount = Math.ceil(cardData.length / pageSize)
    const fetchIdRef = React.useRef(0)

    const fetchData = React.useCallback(({ controlledPageCount, cardData, pageSize, pageIndex}) => {
    ...
    return(
        ...
        <div className="row mt-3">
            ...
            <Pagination
                props={{controlledPageCount, cardData, fetchData, pageIndex, pageSize,
                    canPreviousPage, canNextPage, previousPage, nextPage, gotoPage, pageCount}}
                />
            </div>
            <select className="mr-2" value={pageSize}
                onChange={e => { resetAndSetPageSize(Number(e.target.value)) }}>
                {pageSizeOptions}
            </select>
            <div colSpan="10000">
                Showing {currentData.length} of ~{controlledPageCount * pageSize}{' '} results
            </div>
        </div>  
        </>
    )
